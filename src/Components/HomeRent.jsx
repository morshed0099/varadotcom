...
const HomeRent = ({ rents,rateRange,rentRange1}) => {
    const { user } = useContext(userAuth)
    const navigate=useNavigate()
    console.log(rents, '8');
    useEffect(() => {
        fetch(`https://varadotcom-server.vercel.app/user/${user?.uid}`)
            .then(res => res.json)
    ...
    // console.log(rateRange,rentRange1,rentRange2, '21');
        ...
        <>
            {
                  areaDivison? <>
                    ...
                    <h1 className='text-center mt-12 font-bold text-2xl text-red-800'>{rents.message}</h1>
